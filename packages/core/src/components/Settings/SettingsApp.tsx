...
import { Farming } from '@hydrangea/icons';
import {
...
export default function SettingsApp(props: SettingsAppProps) {
  ...
  async function handleOpenFAQURL(): Promise<void> {
    try {
      ...
      await shell.openExternal('https://github.com/Hydrangea-Network/hydrangea-blockchain/wiki/FAQ');
    } catch (error: any) {
      showError(error);
    }
  }
  ...
  async function handleOpenSendFeedbackURL(): Promise<void> {
    try {
      const shell: Shell = (window as any).shell;
      await shell.openExternal('https://feedback.hydrangea.website/lightwallet');
    } catch (error: any) {
      showError(error);
    }
  }
