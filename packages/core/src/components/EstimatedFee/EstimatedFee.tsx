...
import { useGetFeeEstimateQuery } from '@hydrangea/api-react';
import {
  FormControl,
  IconButton,
  InputLabel,
...
import mojoToHydrangeaLocaleString from '../../utils/mojoToHydrangeaLocaleString';
import Fee from '../Fee';
...
export default function EstimatedFee(props: FeeProps) {
  ...
  const txCostEstimates = {
    walletSendXHG: Math.floor(maxBlockCostCLVM / 1170),
  ...
  function formatEst(number, multiplier, locale) {
    ...
    let formatNum = mojoToHydrangeaLocaleString(num, locale);
    return formatNum;
  }
        ...
        >
          {estList.map((option) => (
            <MenuItem value={String(option.estimate)} key={option.time}>
              ...
              >
                <Flex>
                  <Trans>{option.estimate} TXHG</Trans>
                </Flex>
