...
import { AlertDialog, Card, Flex, hydrangeaToMojo, mojoToHydrangeaLocaleString } from '@hydrangea/core';
import {
  send_transaction,
...
const IncompleteCard = (props) => {
  ...
  function submit() {
    ...
    const hydrangeaper_input = ip_parsed.limit;
    ...
    const hydrangeaper_value = Number.parseInt(Number(hydrangeaper_input));
        ...
        <Grid item xs={12}>
          <Box display="flex">
            ...
            <Box flexGrow={1}>
              <Typography variant="subtitle1">
                <Trans>
                  Spending Limit (hydrangea per interval):{' '}
                  {mojoToHydrangeaLocaleString(limit)}
                </Trans>
              </Typography>
            </Box>
          </Box>
  ...
  return (
    <Grid item xs={12}>
      <Box display="flex">
        ...
        <Box>
          <Typography variant="subtitle1">
            {mojoToHydrangeaLocaleString(props.balance)} {currencyCode}
          </Typography>
        </Box>
      </Box>
    </Grid>
...
const SendCard = (props) => {
  ...
  function send() {
    ...
    const fee = hydrangeaToMojo(fee_input.value);
