...
export default function OfferSummary(props: Props) {
  ...
  const makerAmount = makerEntries.length > 0 ? (['xhg', 'txhg'].includes(makerEntries[0][0].toLowerCase()) ? mojoToChia(makerEntries[0][1]) : mojoToCAT(makerEntries[0][1])) : undefined;
  const takerAmount = takerEntries.length > 0 ? (['xhg', 'txhg'].includes(takerEntries[0][0].toLowerCase()) ? mojoToChia(takerEntries[0][1]) : mojoToCAT(takerEntries[0][1])) : undefined;
